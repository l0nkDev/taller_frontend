...
import { Receipt, Clock, ChefHat, CheckCircle, ShoppingCart, RefreshCcw } from "@tamagui/lucide-icons";
...
export const OrdersView = () => {
  ...
  const [updateOrderDetail, { isLoading: isUpdating }] =
    useUpdateOrderDetailMutation();
    ...
    <YStack p="$5" f={1} overflow="hidden">
      ...
      <ScrollView showsVerticalScrollIndicator={false} f={1}>
        <XStack flexWrap="wrap" gap="$5" pb="$10">
          {activeOrders && visibleOrders.length === 0 ? (
            ...
            visibleOrders.map((order) => {
              const pendingDetails = order.detail.filter(
                (d) => d.status !== "S" && d.status !== "X",
              );
