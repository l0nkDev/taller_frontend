...
function createThemeWithPalettes(palettes2, defaultPalette, definition, options, name, skipCache = false) {
  ...
  return createTheme2(palettes2[defaultPalette], newDef, options, name, skipCache);
}
...
function applyMaskStateless(info, mask, options = {}, parentName) {
  ...
  const maskOptions2 = {
    ...
    ...options,
    skip
  }, template = mask.mask(info.definition, maskOptions2), theme = createTheme2(info.palette, template);
  return {
    ...info,
    cache: /* @__PURE__ */ new Map(),
    definition: template,
...
var createInterFont = /* @__PURE__ */ __name((font = {}, {
  ...
  return (0, import_core3.createFont)({
    ...
    lineHeight: Object.fromEntries(Object.entries(size4).map(([k, v]) => [k, sizeLineHeight((0, import_core3.getVariableValue)(v))])),
    weight: {
      4: "300"
    },
    letterSpacing: {
...
var themes2 = {
  light: {
    ...
    color: color3.gray800,
    borderColor: color3.amber200,
    bgGradientStart: color3.amber50,
    bgGradientEnd: color3.orange50,
    cardBg: color3.white,
    cardBorder: color3.amber200,
    primaryText: color3.gray800,
    secondaryText: color3.gray600,
    brandAccent: color3.amber600,
    brandMain: color3.amber500
  },
  dark: {
    background: "#1a1a1a",
    color: "#f3f4f6",
    ...
    brandAccent: color3.amber500,
    brandMain: color3.orange500
  }
};
var appConfig = createTamagui({
  ...config2,
  animations: config2.animations,
  tokens: tokens3,
  shorthands,
  themes: themes2
});
